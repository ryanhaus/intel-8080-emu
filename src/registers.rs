...
use std::{convert::{ From, TryFrom }, error::Error};
...
impl RegisterArray {
    ...
    pub fn write_reg(&mut self, register: Register, value: RegisterValue) -> Result<(), String> {
        ...
        match register {
            ...
            _ => return Err(format!("Cannot write to {}", register.get_human_readable_name()))
        };
...
mod tests {
    ...
    fn register_array_read_write() {
        ...
        registers_8.iter()
            ...
            .for_each(|(reg, val)|
                reg_array.write_reg(*reg, RegisterValue::from(*val)).unwrap()
            );
        ...
        registers_and_pairs
            .map(|(a, (b, c))| 
                (
                    ...
                    (
                        reg_array.read_reg(b).into(),
                        reg_array.read_reg(c).into(),
                    )
                )
            )
            .into_iter()
